...
import { rpcBatch, STABLECOINS, NADO_LOGO, TYDRO_DATA_PROVIDER } from '@/lib/ink'
import { kvGet, kvSet } from '@/lib/kvCache'
import { log, redactError } from '@/lib/log'
import { TIMEOUT } from '@/lib/constants'
import type { VfatFarm, InkyPair } from '@/lib/defi/types'
...
async function fetchVelodromeData(): Promise<AprEntry[]> {
  ...
  try {
    ...
    if (!res.ok) {
      log.error('best-aprs', `vfat API HTTP ${res.status}`)
      return []
    }
    farms = await res.json()
    if (!Array.isArray(farms)) return []
  } catch (e) {
    log.error('best-aprs', 'vfat fetch failed', redactError(e))
    return []
  }

  const out: AprEntry[] = []

  // Match VfatFarm.pool with optional isStable
  interface VfatPool { isStable?: boolean; underlying?: VfatFarm['pool'] extends infer P ? P extends { underlying?: infer U } ? U : never : never }
  for (const farm of farms) {
    ...
    // some farms expose isKilled flag we want to skip when present
    if ((farm as unknown as { isKilled?: boolean }).isKilled) continue

    const pool        = (farm.pool ?? {}) as VfatPool & VfatFarm['pool']
    const underlying  = (pool?.underlying ?? []) as Array<{
      symbol?:         string
      price?:          number
      decimals?:       number
      stakedReserve?:  string
      reserve?:        string
      reserves?:       string
      dailySwapFees?:  string
    }>
    ...
    for (const r of (farm.rewards ?? [])) {
      ...
      const xveloPrice = r.rewardToken?.price ?? 0
    ...
    const tokens  = underlying.map(t => (t.symbol ?? '').replace('₮', 'T'))
...
async function fetchInkySwapData(): Promise<AprEntry[]> {
  ...
  try {
    ...
    if (!res.ok) {
      log.error('best-aprs', `InkySwap HTTP ${res.status}`)
      return out
    }
    const pairs: (InkyPair & { version?: string; fee_tier?: string })[] = await res.json()
    if (!Array.isArray(pairs)) return out

    for (const p of pairs) {
      const base  = (p.token0?.symbol ?? '').replace('\u20ae', 'T')
      const quote = (p.token1?.symbol ?? '').replace('\u20ae', 'T')
      if (!base || !quote) continue

      const tvl = Number(p.liquidity_usd ?? 0)
      ...
      const ver     = p.version  ?? 'v2'
      const feeTier = p.fee_tier ?? ''
  ...
  } catch (e) { log.error('best-aprs', 'InkySwap error', redactError(e)) }
  return out
}
...
async function fetchNadoVault(): Promise<AprEntry[]> {
  try {
    ...
    if (!res.ok) {
      log.error('best-aprs', `Nado archive HTTP ${res.status}`)
      return []
    }

    interface NadoSnapshot { tvl?: string; oracle_price_x18?: string }
    const data              = await res.json()
    const snaps: NadoSnapshot[] = data?.snapshots ?? []
    ...
    log.error('best-aprs', 'Nado error', redactError(e))
    return []
  }
}
...
async function fetchTydroData(): Promise<AprEntry[]> {
  ...
  // rpcBatch now does the primary -> secondary failover internally.
  const [rpcResults, merklRes] = await Promise.allSettled([
    rpcBatch(batch, TIMEOUT.SLOW),
    fetch(MERKL_URL, {
      headers: { Accept: 'application/json' },
      signal: AbortSignal.timeout(TIMEOUT.NORMAL),
  ...
  if (rpcResults.status === 'fulfilled' && Array.isArray(rpcResults.value)) {
    for (const item of rpcResults.value) {
      const idx = typeof item.id === 'number' ? item.id : Number(item.id)
      const reserve = TYDRO_RESERVES[idx]
    ...
    log.error('best-aprs', 'Tydro on-chain failed (both RPCs)',
      rpcResults.status === 'rejected' ? redactError(rpcResults.reason) : 'empty from both RPCs')
  }

  // Merkl: additional incentive APR per reserve token address
  interface MerklOpportunity { apr?: number; tokens?: { address?: string }[] }
  const merklAprs = new Map<string, number>()
  if (merklRes.status === 'fulfilled' && Array.isArray(merklRes.value)) {
    for (const opp of (merklRes.value as MerklOpportunity[])) {
...
async function fetchCurveData(): Promise<AprEntry[]> {
  ...
  } catch (e) { log.error('best-aprs', 'Curve error', redactError(e)) }
  return out
}
...
async function fetchAllAprs(): Promise<AprEntry[]> {
  ...
  if (v.status === 'rejected') log.error('best-aprs', 'Velodrome failed', redactError(v.reason))
  if (i.status === 'rejected') log.error('best-aprs', 'InkySwap failed',  redactError(i.reason))
  if (n.status === 'rejected') log.error('best-aprs', 'Nado failed',      redactError(n.reason))
  if (t.status === 'rejected') log.error('best-aprs', 'Tydro failed',     redactError(t.reason))
  if (c.status === 'rejected') log.error('best-aprs', 'Curve failed',     redactError(c.reason))
...
export async function GET() {
  ...
  try {
    ...
    log.error('best-aprs', 'fatal error', redactError(e))
