...
import { rpcBatch, getEthPrice } from '@/lib/ink'
...
async function fetchNFTsViaOpenSea(address: string, apiKey: string, ethPrice: number) {
  ...
  const nfts = items.map((nft: any) => {
    ...
    // Sanitize all user-visible strings from OpenSea — names/symbols are
    // controlled by NFT issuers and can contain HTML or injection payloads.
    const fallbackCollection = `${contract.slice(0, 6)}...${contract.slice(-4)}`
    return {
      id:          `${contract}_${tokenId}`,
      contract,
      tokenId,
      collection:  sanitizeName(nft.collection ?? nft.name) || fallbackCollection,
      symbol:      '',
      name:        sanitizeName(nft.name) || `#${tokenId}`,
...
async function fetchTokenMeta(uri: string) {
  try {
    ...
    const r = await fetch(url, { signal: AbortSignal.timeout(TIMEOUT.FAST) })
...
export async function GET(req: NextRequest) {
  ...
  if (openSeaKey) {
    try {
      ...
      log.error('nfts', 'opensea error', redactError(e))
  ...
  try {
    ...
    const nfts = await Promise.all(owned.map(async ({ contract, tokenId }, i) => {
      ...
      return {
        ...
        collection: sanitizeName(cm.name) || fallbackCollection,
        symbol:     sanitizeName(cm.symbol),
    ...
    log.error('nfts', 'blockscout fallback error', redactError(e))
    return NextResponse.json({ nfts: [], nftValue: 0, total: 0 })
  }
}
