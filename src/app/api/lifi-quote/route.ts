...
import { TIMEOUT, CHAIN_ID_RE } from '@/lib/constants'
import { isEvmAddress, isChainId } from '@/lib/validation'
...
// silence unused import warning while keeping isChainId available for future use
void isChainId
...
export async function GET(req: NextRequest) {
  ...
  try {
    const res = await fetch(`${LIFI_API}/quote?${params}`, {
      ...
      signal: AbortSignal.timeout(TIMEOUT.LIFI),
    })
    ...
    log.error('lifi-quote', 'upstream error', redactError(e))
