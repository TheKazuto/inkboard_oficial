...
function getClientIp(req: NextRequest): string {
  // cf-connecting-ip is the only trustworthy IP source on Cloudflare —
  // it is set by the edge and cannot be spoofed by the client.
  // x-forwarded-for can be set by anyone, so we only accept it in dev.
  ...
  // No CF header in production = request bypassed the edge. Treat as one
  // bucket so an attacker can't burn through rate limits by forging headers.
  return 'no-cf-ip'
}
