 ...
 *
 * Usage:
 *   import { kvGet, kvSet, kvGetSWR } from '@/lib/kvCache'
 *
 *   // Manual form — caller decides what to do with stale data
 ...
 *   await kvSet('best-aprs', freshData, 600)
 *
 *   // Stale-while-revalidate form — one-liner
 *   const data = await kvGetSWR('best-aprs', 3 * 60_000, 600, fetchUpstream)
 */
...
export async function kvGet<T>(key: string, softTtlMs: number): Promise<CacheResult<T>> {
  ...
  try {
    ...
    log.error('kvCache', `KV get(${key}) failed, using memStore`, e)
  }
...
export async function kvSet<T>(key: string, data: T, hardTtlSec: number): Promise<void> {
  ...
  try {
    ...
    log.error('kvCache', `KV set(${key}) failed, memStore still updated`, e)
  }
}
 ...
 *
 * 1. If a fresh entry exists, return it immediately.
 * 2. Otherwise call `fetcher`, persist the result, and return it.
 * 3. If `fetcher` fails and a stale entry exists, return the stale value.
 * 4. If `fetcher` fails and no entry exists, rethrow.
 *
 * @param key         Cache key
 * @param softTtlMs   Freshness window (ms)
 * @param hardTtlSec  KV TTL (seconds) — should be >= 2x softTtl
 * @param fetcher     Upstream loader called only when cache is stale or missing
 */
export async function kvGetSWR<T>(
  key:        string,
  softTtlMs:  number,
  hardTtlSec: number,
  fetcher:    () => Promise<T>,
): Promise<T> {
  const cached = await kvGet<T>(key, softTtlMs)
  if (cached.data && cached.fresh) return cached.data
  try {
    const fresh = await fetcher()
    await kvSet(key, fresh, hardTtlSec)
    return fresh
  } catch (e) {
    if (cached.data) {
      log.warn('kvCache', `fetcher failed for ${key}, serving stale`, e)
      return cached.data
    }
    throw e
  }
